			...
			<div class="box box-success">
				...
				<div class="box-body">
					<table id="actores" class="table table-bordered table-striped">
						...
						<tbody>
							@foreach($tutoreestudiantes as $tutoreestudiante)
								<tr>
									<td>
										{{$tutoreestudiante -> id }}
									</td>
									<td>
										{{$tutoreestudiante -> estudiente -> nombre}}
									</td>
									<td>
										{{$tutoreestudiante -> estudiante_id }}
									</td>
									<td>
										{{ Form::open(['method' => 'get', 'route' => ['tutoreestudiantes.show', $tutoreestudiante -> id]]) }}
											{{ Form::button('', ['type' => 'submit', 'class' => 'btn btn-primary btn-block fa fa-eye'] )  }}
										{{ Form::close() }}
									</td>
									<td>
										{{ Form::open(['method' => 'get', 'route' => ['tutoreestudiantes.edit', $tutoreestudiante -> id]]) }}
											{{ Form::button('', ['type' => 'submit', 'class' => 'btn btn-success btn-block fa fa-pencil'] )  }}
										{{ Form::close() }}
									</td>
									<td>
										{{ Form::open(['method' => 'delete', 'route' => ['tutoreestudiantes.destroy', $tutoreestudiante -> id]]) }}
											{{ Form::button('', ['type' => 'submit', 'class' => 'btn btn-danger btn-block fa fa-trash'] )  }}
										{{ Form::close() }}
									</td>
								</tr>
							@endforeach
